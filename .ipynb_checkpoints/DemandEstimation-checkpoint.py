...
class endog_data(): 
    ...
    def Consumer_demand(self, theta, X, P, sigma): 
        ...
        J = S.shape[0]
        dSdp = np.diag(alpha * S * ( 1 - S)) + (np.identity(J) - 1) * alpha * S**2    # -alpha * S**2
        return S, dSdp
    ...
    def Oligo_prices(self, f:int,mc: np.array, S, dSdp, product_map):
        ...
        P = mc + inv(Delta) @ S # (mc includes omega)

        return P


    def c_map(self, P, theta, X, mc, product_map, f, sigma):
        # First step: Calculate market shares:
        S, dSdp = self.Consumer_demand(theta, X, P, sigma) 
        # Second step: Calculate new prices:
        try:
            P = self.Oligo_prices(f, mc, S, dSdp, product_map)
            ...
            P = self.c_map(P, theta, X, mc, product_map, f, sigma)
    ...
    def sim_data(self, P, theta, X, mc, product_map, f, J, t=0):
        
        c_map_l = lambda P: self.c_map(P, theta, X, mc, product_map, f, self.sigma[:,t])
        ...
        S, dSdp = self.Consumer_demand(theta, X, P, self.sigma[:,t]) 
    ...
    def sim_data_T(self, X, f, j, T, theta, gamma, cov, product_map, disable = False):
        ...
        for t in tqdm(range(T), disable = disable): 
            inv_true = False
            while not inv_true: 
                P[:,t],S[:,t], inv_true=self.sim_data(P0, theta, X, mc[:,t], product_map, f, j, t)
